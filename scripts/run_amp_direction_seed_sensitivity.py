#!/usr/bin/env python3
"""Run a small seed-sensitivity test for amplitude-direction Seidel recovery."""
...
def make_sweep_args(args: argparse.Namespace, parameterization: str) -> SimpleNamespace:
...
def main() -> None:
    ...
    for parameterization in args.parameterizations:
        if parameterization not in {"amp_direction", "amp_direction_detach_norm"}:
            raise ValueError(f"Unsupported parameterization for this test: {parameterization}")

    candidates = make_candidates([args.direction], [args.target_rms], seidel_convention="classical6d")
    if len(candidates) != 1:
        raise RuntimeError("Expected exactly one candidate")
    base_candidate = candidates[0]

    all_cases: list[tuple[int, str]] = []
    for seed in args.seeds:
        for parameterization in args.parameterizations:
            all_cases.append((int(seed), parameterization))
    ...
    print(
        ...
        f"direction={args.direction} rms={args.target_rms:g}",
        flush=True,
    )

    for seed, parameterization in selected_cases:
        candidate = replace(base_candidate, candidate_id=f"{base_candidate.candidate_id}__{parameterization}")
        run_case(
            ...
            sweep_args=make_sweep_args(args, parameterization),
        )
