...
RUN_PREFIX="pretrain_pairs_tunedprior_noskip6x128_$(date +%Y%m%d)"
IMAGE="fluorescence"
SIZE="256"
MODES=("joint" "frozen")
PAIRS=("400:5" "600:5" "800:5" "400:7.5" "600:7.5" "400:10")
NUM_ITER="1000"
GT_PRESET="ucla"
...
usage() {
  cat <<'USAGE'
Usage: run_pretrain_ablation_pairs.sh [options]

Runs selected (pretrain-iter, pretrain-scalar) pairs while keeping object
...
Options:
  ...
  --pairs ITER:SCALAR...     Selected pairs. Default: 400:5 600:5 800:5 400:7.5 600:7.5 400:10.
...
if [[ ${#PAIRS[@]} -eq 0 ]]; then
  echo "--pairs must contain at least one ITER:SCALAR pair" >&2
  exit 2
fi
...
for pair in "${PAIRS[@]}"; do
  if [[ "$pair" != *:* ]]; then
    echo "Invalid pair '${pair}'; expected ITER:SCALAR" >&2
    exit 2
  fi
  pretrain_iter="${pair%%:*}"
  pretrain_scalar="${pair#*:}"
