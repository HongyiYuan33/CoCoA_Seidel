"""Compare scalar5 baseline, post-pretrain 400 variants, and post-pretrain 1000 variants."""
...
DEFAULT_OUT = OUTPUT_ROOT / "secondjoint_postpre1000_architecture_comparison_20260613"
DEFAULT_RCP_DIRS = [
    ...
    OUTPUT_ROOT / "secondjoint_postpre1000_4d_size256_three_images_rms020_030_040_pre400_joint1000x2_20260613_rcp_stats",
]
BASELINE = "single_joint"
METHOD_TO_VARIANT = {
    "scalar5_single_joint": "single_joint",
    "scalar5_second_joint": "second_joint",
    "scalar5_second_joint_postobjraw_scalar5": "postobjraw_scalar5_postpre400",
    "scalar5_second_joint_postobjraw_pg_p0p1_p99p9_g1p5": "postobjraw_pg_postpre400",
    "scalar5_second_joint_postreconpct_keepobj": "postreconpct_keepobj_postpre400",
    "scalar5_second_joint_postreconpct_resetobj": "postreconpct_resetobj_postpre400",
    "scalar5_second_joint_postobjraw_scalar5_postpre1000": "postobjraw_scalar5_postpre1000",
    "scalar5_second_joint_postobjraw_pg_p0p1_p99p9_g1p5_postpre1000": "postobjraw_pg_postpre1000",
    "scalar5_second_joint_postreconpct_keepobj_postpre1000": "postreconpct_keepobj_postpre1000",
    "scalar5_second_joint_postreconpct_resetobj_postpre1000": "postreconpct_resetobj_postpre1000",
}
VARIANT_ORDER = [
    "single_joint",
    "second_joint",
    "postobjraw_scalar5_postpre400",
    "postobjraw_scalar5_postpre1000",
    "postobjraw_pg_postpre400",
    "postobjraw_pg_postpre1000",
    "postreconpct_keepobj_postpre400",
    "postreconpct_keepobj_postpre1000",
    "postreconpct_resetobj_postpre400",
    "postreconpct_resetobj_postpre1000",
]
VARIANT_LABELS = {
    "single_joint": "baseline: scalar5 single joint",
    "second_joint": "second joint reset Seidel",
    "postobjraw_scalar5_postpre400": "post object-raw scalar5, post-pretrain 400",
    "postobjraw_scalar5_postpre1000": "post object-raw scalar5, post-pretrain 1000",
    "postobjraw_pg_postpre400": "post object-raw p/g, post-pretrain 400",
    "postobjraw_pg_postpre1000": "post object-raw p/g, post-pretrain 1000",
    "postreconpct_keepobj_postpre400": "post recon percentile keep, post-pretrain 400",
    "postreconpct_keepobj_postpre1000": "post recon percentile keep, post-pretrain 1000",
    "postreconpct_resetobj_postpre400": "post recon percentile reset, post-pretrain 400",
    "postreconpct_resetobj_postpre1000": "post recon percentile reset, post-pretrain 1000",
}
METRICS = [
...
def load_rows(rcp_dirs: list[Path]) -> list[dict[str, Any]]:
    ...
    for rcp_dir in rcp_dirs:
        ...
        for row in read_csv(comp_path):
            ...
            item["variant_label"] = VARIANT_LABELS[variant]
...
def validate_rows(rows: list[dict[str, Any]]) -> None:
    ...
    if len(groups) != 9 or bad:
        raise RuntimeError(f"Expected 9 complete image/RMS groups; bad={bad}")
...
def summary_vs_baseline(delta_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for variant in VARIANT_ORDER:
        ...
        for metric, _label, _higher in METRICS:
            record[f"baseline_{metric}_mean"] = mean(
                [parse_float(row[f"baseline_{metric}"]) for row in group]
            )
            record[f"variant_{metric}_mean"] = mean(
                [parse_float(row[f"variant_{metric}"]) for row in group]
            )
            record[f"delta_{metric}_mean"] = mean(
                [parse_float(row[f"delta_{metric}"]) for row in group]
            )
            record[f"improves_{metric}_count"] = sum(
                1 for row in group if str(row[f"improves_{metric}"]).lower() == "true"
            )
        out.append(record)
    return out
...
def best_variant_by_case(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for image in IMAGES:
        for rms in RMS_VALUES:
            ...
            for metric, _label, higher in METRICS:
                best = (
                    max(group, key=lambda row: parse_float(row.get(metric)))
                    if higher
                    else min(group, key=lambda row: parse_float(row.get(metric)))
                )
...
def plot_variant_mean_metrics(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(1, len(METRICS), figsize=(30, 5.2), constrained_layout=True)
    for ax, (metric, label, higher) in zip(axes, METRICS):
        ...
        ax.set_xticks(range(len(VARIANT_ORDER)), VARIANT_ORDER, rotation=40, ha="right", fontsize=7)
        ax.grid(axis="y", alpha=0.25)
        for idx, value in enumerate(values):
            ax.text(idx, value, fmt(value), ha="center", va="bottom", fontsize=6)
...
def plot_variant_metrics_by_image_rms(rows: list[dict[str, Any]], out_dir: Path) -> None:
    image_rms_dir = out_dir / "figures" / "by_image_rms"
    image_rms_dir.mkdir(parents=True, exist_ok=True)
    colors = ["#5B6770" if variant == BASELINE else "#4C78A8" for variant in VARIANT_ORDER]
    lookup = build_lookup(rows)
    for image in IMAGES:
        image_dir = image_rms_dir / image
        image_dir.mkdir(parents=True, exist_ok=True)
        for rms in RMS_VALUES:
            fig, axes = plt.subplots(1, len(METRICS), figsize=(32, 5.3), constrained_layout=True)
            fig.suptitle(f"{image}: RMS {rms:g}", fontsize=13, fontweight="bold")
            for ax, (metric, label, higher) in zip(axes, METRICS):
                values = [
                    parse_float(lookup[(image, round(rms, 6), variant)].get(metric))
                    for variant in VARIANT_ORDER
                ]
                ax.bar(range(len(VARIANT_ORDER)), values, color=colors)
                ax.set_title(f"{label}\n{'higher better' if higher else 'lower better'}")
                ax.set_xticks(range(len(VARIANT_ORDER)), VARIANT_ORDER, rotation=42, ha="right", fontsize=7)
                ax.grid(axis="y", alpha=0.25)
                for idx, value in enumerate(values):
                    ax.text(idx, value, fmt(value), ha="center", va="bottom", fontsize=6)
            fig.savefig(image_dir / f"variant_metrics__{image}__{rms_tag(rms)}.png", dpi=180)
            plt.close(fig)
...
def plot_delta_heatmaps(delta_rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    variants = [variant for variant in VARIANT_ORDER if variant != BASELINE]
    for metric, label, higher in METRICS:
        grid = np.full((len(variants), len(case_labels)), np.nan, dtype=np.float64)
        for row in delta_rows:
            i = variants.index(str(row["variant"]))
            j = IMAGES.index(str(row["image"])) * len(RMS_VALUES) + RMS_VALUES.index(
                round(parse_float(row["target_rms"]), 2)
            )
        ...
        fig, ax = plt.subplots(figsize=(16.5, 6.0), constrained_layout=True)
        ...
        ax.set_yticks(np.arange(len(variants)), variants, fontsize=7)
...
def write_summary_md(out_dir: Path, variant_summary: list[dict[str, Any]], best_rows: list[dict[str, Any]]) -> None:
    lines = [
        "# Scalar5 Post-Pretrain 1000 Architecture Comparison",
        "",
        "Baseline: `scalar5_single_joint`; deltas compare each variant against that baseline.",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--copy-rcp", action="store_true")
    ...
    baseline_summary = summary_vs_baseline(delta_rows)
    best_rows = best_variant_by_case(rows)
    winner_rows = winner_counts(best_rows, baseline_summary)
    ...
    write_csv(baseline_summary, args.output_dir / "summary_vs_baseline_by_variant.csv")
    ...
    plot_variant_metrics_by_image_rms(rows, args.output_dir)
