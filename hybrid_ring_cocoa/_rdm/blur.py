...
def ring_convolve_full(
    ...
    Notes
    -----
    When ``psf_roft.requires_grad`` is True (joint Seidel-identification
    setting), the per-radius accumulation loop holds 768 iterations of complex
    intermediates simultaneously, peaking at ~30 GB and OOMing the 49 GB Ada.
    The trainable branch wraps each iteration in ``torch.utils.checkpoint``
    so intermediates are released after each forward and recomputed during
    backward — peak memory drops to a few GB at the cost of ~2× joint-phase
    wall time. The oracle path (frozen PSF, no grad) keeps the original
    in-place loop for performance.
    """
    ...
    use_checkpoint = bool(psf_roft.requires_grad)

    if use_checkpoint:
        half = psf_roft.shape[1] // 2
        chunk_size = 64  # Same trade-off as the PSF-synthesis chunking — see
                         # ``hybrid_ring_cocoa.optics.seidel_psf._compute_roft_psfs_checkpointed``.

        def _chunk_contrib(psf_roft_chunk: torch.Tensor,
                           obj_fft_chunk: torch.Tensor,
                           ia_chunk: torch.Tensor) -> torch.Tensor:
            """Sum contributions from ``chunk_size`` consecutive radii.

            psf_roft_chunk: (K, 2H, samples)
            obj_fft_chunk: (1537, K) complex (slice of obj_fft)
            ia_chunk: (K,) float
            """
            real_part = psf_roft_chunk[:, 0:half, :]
            imag_part = psf_roft_chunk[:, half:, :]
            curr_psf_polar_fft = real_part + 1j * imag_part
            # Broadcast: (K,) * ((1537, K, 1) * (K, 2H/2, samples))
            scaled = ia_chunk.view(-1, 1, 1) * curr_psf_polar_fft  # (K, half, samples) complex
            # Multiply by obj_fft column for each radius and sum.
            # obj_fft_chunk: (1537, K) complex. We need (1537, samples) result.
            # einsum: 'fk,khs->fhs' would give (1537, half, samples). Then sum over h?
            # Actually we want: out[f, s] += sum_k obj_fft[f, k] * scaled[k, ?, s]
            # Looking at original loop:
            #   img_polar_fft += integration_area * (obj_fft[:, index][:, None] * curr_psf_polar_fft)
            #   curr_psf_polar_fft is (half, samples). obj_fft[:, idx][:, None] is (1537, 1).
            #   Product: (1537, samples). So img_polar_fft is (1537, samples).
            # That means half == 1537! (psf_roft.shape[1]//2 = (samples*4+2)/2 = samples*2+1)
            # And dim 0 of img_polar_fft is samples*2+1 = half.
            # So contribution per radius is (half, samples).
            # Chunked: out[h, s] = sum_k obj_fft[h, k] * ia[k] * curr_psf[k, h, s]
            return torch.einsum("hk,khs->hs", obj_fft_chunk, scaled)

        img_polar_fft = torch.zeros_like(obj_fft, dtype=torch.complex64, device=device)
        ia_full = torch.as_tensor(
            r_list * dr * dtheta, dtype=torch.float32, device=device,
        )
        for start in range(0, num_radii, chunk_size):
            end = min(start + chunk_size, num_radii)
            psf_roft_chunk = psf_roft[start:end]
            obj_fft_chunk = obj_fft[:, start:end]
            ia_chunk = ia_full[start:end]
            contrib = checkpoint(
                _chunk_contrib,
                psf_roft_chunk,
                obj_fft_chunk,
            ...
            img_polar_fft = img_polar_fft + contrib
    else:
